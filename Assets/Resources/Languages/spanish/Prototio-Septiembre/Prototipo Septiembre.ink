 ...
 ~ temp group2_speaker = LIST_RANDOM(LIST_ALL(group2_members))
   {
    ...
    {group2_speaker}: ¿Has leído esto antes de enviarlo?
    - checked == true && this_news_read == false:
    {group2_speaker}: ¿Pero tu te crees esto? 
    - else:
    {group2_speaker}: Interesante, gracias. 
    }
...
{ 
    - news_read < news_fake:
    {group2_speaker}: Creo que nos mandas cosas antes de leerlas. 
    - else:
    {group2_speaker}: Gracias por compartir. 
    }


// This function is for group 2 to notice that the player is sending mostly news that have not been factchecked
=== function scold3_group2()
 ~ temp group2_speaker = LIST_RANDOM(LIST_ALL(group2_members))
{ 
    - news_checked < news_count+1 : 
    {group2_speaker}: Estos artículos no han sido contrastados. Necesitas leer con más cuidado y ver de dónde han salido las noticias. 
    - else:
    {group2_speaker}:Luego lo leo. 
    }

// This function is for group 3 to notice that the player is sending mostly news that have not been factchecked
=== function scold4_group3()
 ~ temp group3_speaker = LIST_RANDOM(LIST_ALL(group3_members))
{ 
    - news_checked < news_count+1 : 
    {group3_speaker}: Tanto artículo me confunde. Esto suena a inventado, ¿de dónde ha salido? 
    - else:
    {group3_speaker}: Interesante, gracias. 
    }
